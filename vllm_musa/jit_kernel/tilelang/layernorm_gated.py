...
def _rms_norm_gated_kernel_cta(
    ...
    return musa_rms_norm_gated_cta
...
def _pytorch_gated_norm(
    x, weight, bias, eps, z, group_size, norm_before_gate, is_rms_norm, activation
):
    """MUSA: pure-PyTorch general-case gated (layer/rms) norm fallback, used when
    the fused TileLang fast path does not apply. Self-contained (no external dep)."""
    ...
    def _norm(v):
        if group_size is None or group_size == v.shape[-1]:
            if is_rms_norm:
                return v * torch.rsqrt(v.pow(2).mean(-1, keepdim=True) + eps)
            m = v.mean(-1, keepdim=True)
            return (v - m) * torch.rsqrt((v - m).pow(2).mean(-1, keepdim=True) + eps)
        M, N = v.shape
        vg = v.reshape(M, N // group_size, group_size)
        if is_rms_norm:
            vn = vg * torch.rsqrt(vg.pow(2).mean(-1, keepdim=True) + eps)
        else:
            m = vg.mean(-1, keepdim=True)
            vn = (vg - m) * torch.rsqrt((vg - m).pow(2).mean(-1, keepdim=True) + eps)
        return vn.reshape(M, N)

    out = _norm(xf) * weight.float()
    if bias is not None:
        out = out + bias.float()
    if norm_before_gate:
        out = _gate(out)
